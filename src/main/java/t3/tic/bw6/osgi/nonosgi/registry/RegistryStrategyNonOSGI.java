...
public class RegistryStrategyNonOSGI extends RegistryStrategy {
	...
	public RegistryStrategyNonOSGI(File[] storageDirs, boolean[] cacheReadOnly,	Object token, ClassLoader classLoader) {
	...
	private boolean loadPluginXML(URL pluginManifest, ExtensionRegistry extensionRegistry, Map<String, URL> manifests) {
		...
		if (!extensionRegistry.addContribution(is, contributor, true, pluginManifest.getPath(), translationBundle, token, timestamp)) {
			if (DebugHelper.DEBUG) {
				DebugHelper.logError("<plugin.xml> [ERROR] : (" + pluginManifest.getPath() + "): ", 1);
